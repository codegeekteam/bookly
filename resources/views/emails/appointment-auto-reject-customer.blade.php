    ...
    <div style="direction: rtl; text-align: right;">
        ...
        <div class="section" style="direction: rtl; text-align: right;">
            ...
            <div class="info-line" style="text-align: right;">
                ...
                <span class="value">Your appointment #  {{ $appointment->id }}  with {{ $appointment->serviceProvider->name }} on {{ \Carbon\Carbon::parse($serviceDate)->format('l') }} & {{ \Carbon\Carbon::parse($serviceDate)->format('d-m-Y') }} & {{ $appointment->services[0]->pivot->start_time }} has been cancelled due to no action. You will be refunded the full deposit amount of  SAR {{ $appointment->deposit_amount }} to your bank account within 7 days. <br>تم إلغاء موعدك رقم {{ $appointment->id }} مع {{ $appointment->serviceProvider->name }} بتاريخ {{ \Carbon\Carbon::parse($serviceDate)->format('l') }} و {{ \Carbon\Carbon::parse($serviceDate)->format('d-m-Y') }} و {{ $appointment->services[0]->pivot->start_time }} لعدم اتخاذ أي إجراء. سيتم رد مبلغ التأمين بالكامل وقدره {{ $appointment->deposit_amount }} ريال سعودي إلى حسابك البنكي خلال 7 أيام</span>
            </div>
